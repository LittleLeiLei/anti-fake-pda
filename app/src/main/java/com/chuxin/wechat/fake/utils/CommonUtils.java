...
public class CommonUtils {
    ...
    public static DisplayMetrics getDisplayMetrics() {
        return MyApp.get().getResources().getDisplayMetrics();
    }
    ...
    public static boolean hasInternet() {
        ConnectivityManager cm = (ConnectivityManager) MyApp.get()
    ...
    public static boolean isPortrait() {
        return MyApp.get().getResources().getConfiguration()
    ...
    public static int getVersionCode() {
        return getVersionCode(MyApp.get().getPackageName());
    }
    ...
    public static String getVersionName() {
        try {
            return MyApp
                    .get()
                    .getPackageManager()
                    .getPackageInfo(MyApp.get().getPackageName(), 0)
                    .versionName;
        } catch (PackageManager.NameNotFoundException ex) {
    ...
    public static void doubleBack2Finish (Context context, long timestamp) {
        if (lastTimeStamp != 0 && timestamp - lastTimeStamp <= 2000) {
            ((Activity)context).finish();
            // MyApp.get().exit();
        } else {
            lastTimeStamp = timestamp;
            MyApp.get().toast("再次点击返回退出应用");
        }
    }

    public static String wrapOverText(String text, int length) {
        String wrapText = "";
        if (text.length() > length) {
            int substrCount = length / 2;
            wrapText += text.substring(0, substrCount);
            wrapText += "...";
            wrapText += text.substring(text.length() - (length - substrCount));
        } else {
            wrapText = text;
        }
        return wrapText;
